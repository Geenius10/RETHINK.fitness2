...
function backgroundTimerNotice(text){if(document.visibilityState!=="hidden"||!("Notification" in window)||Notification.permission!=="granted")return;navigator.serviceWorker?.ready.then(r=>r.showNotification("RETHINK.fitness",{body:text,tag:"rethink-timer",renotify:true})).catch(()=>{})}
...
function exerciseOptionFieldsMarkup(e,prefix){
 ...
 const equipment=meta.showEquipment?`<div class="form-field option-peer"><label>HILFSMITTEL / GERÄT</label><select id="${prefix}Equipment" class="field"><option value="" ${e.equipmentChoice?"":"selected"}>—</option>${meta.equipment.map(v=>`<option value="${esc(v)}" ${e.equipmentChoice===v?"selected":""}>${esc(v)}</option>`).join("")}</select></div>`:"";
...
function captureExerciseOptionFields(e,prefix){
 ...
 if(meta.showEquipment&&meta.equipmentRequired&&!e.equipmentChoice&&!e._equipmentExplicit){e.equipmentChoice=meta.defaultEquipment||meta.equipment[0]||""}
...
function exerciseDisplayName(e){
 const meta=catalogOptionMeta(e),base=canonicalExerciseName(e?.name||"Übung"),variant=String(e?._variantExplicit?(e?.variant||""):(e?.variant||e?.defaultVariant||meta.catalog?.defaultVariant||"")).trim(),source=(e?.custom&&e?.nameSource)?e.nameSource:(meta.nameSource||"equipment");
 if(source==="variant"){
  if(!variant)return base;
  return `${variant} ${base}`
 }
 if(source==="none")return base;
 const eq=meta.showEquipment?String(e?._equipmentExplicit?(e?.equipmentChoice||""):(e?.equipmentChoice||e?.defaultEquipment||meta.defaultEquipment||e?.equipment?.[0]||"")).trim():"";if(!eq||eq==="-")return base;
...
function exerciseInlineMeta(e){
 ...
 const variant=String(e?._variantExplicit?(e?.variant||""):(e?.variant||meta.catalog?.defaultVariant||"")).trim();
 const eq=meta.showEquipment?String(e?._equipmentExplicit?(e?.equipmentChoice||""):(e?.equipmentChoice||meta.defaultEquipment||"")).trim():"";
...
document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="hidden"){saveAll();captureTabUiState(currentTab);persistUI({capture:false})}else{syncBackgroundTimeSets();resumeVisibleTimeTimers();restoreRestTimer();if(activeWorkout&&document.getElementById("livePage")&&!document.getElementById("livePage").classList.contains("hidden"))renderLive()}});
...
function syncBackgroundTimeSets(){
 if(!activeWorkout)return;
 let changed=false;
 activeWorkout.exercises.forEach((e,ei)=>(e.liveSets||[]).forEach((s,si)=>{
  if(e.measureMode!=="time"||!s._timerRunning||!Number(s._timerEndAt))return;
  const left=Math.max(0,Math.ceil((Number(s._timerEndAt)-Date.now())/1000));
  s.time=left;
  if(left<=0){s._timerRunning=false;s._timedOnce=true;s.achievedTime=timeOriginFor(e,s);s.time=timeOriginFor(e,s);s._timerEndAt=0;changed=true}
 }));
 if(changed)saveAll();
}
function resumeVisibleTimeTimers(){
 if(!activeWorkout)return;
 syncBackgroundTimeSets();
 activeWorkout.exercises.forEach((e,ei)=>(e.liveSets||[]).forEach((s,si)=>{
  if(e.measureMode!=="time"||!s._timerRunning||!Number(s._timerEndAt))return;
  const id=`${ei}|${si}`;if(timeSetTimers.has(id))return;
  const tick=()=>{
   const left=Math.max(0,Math.ceil((Number(s._timerEndAt)-Date.now())/1000));s.time=left;
   const inp=document.querySelector(`#liveBody [data-input="${ei}|${si}|time"]`);if(inp)inp.value=formatTime(left);
   if(left<=0){clearInterval(timeSetTimers.get(id));timeSetTimers.delete(id);s._timerRunning=false;s._timedOnce=true;s.achievedTime=timeOriginFor(e,s);s.time=timeOriginFor(e,s);s._timerEndAt=0;saveAll();signalTone();backgroundTimerNotice("Zeit beendet");renderLive()}
  };tick();timeSetTimers.set(id,setInterval(tick,250));
 }));
}
function toggleTimeTimer(key,btn){
 ...
 if(timeSetTimers.has(id)){
   ...
   s.achievedTime=achieved;s.time=origin;s._timerOrigin=origin;s._timerRunning=false;s._timerEndAt=0;s._timedOnce=true;saveAll();renderLive();
 ...
 s._timerOrigin=origin;s.time=origin;s._touched=true;s._timerRunning=true;s._timerEndAt=Date.now()+origin*1000;saveAll();try{document.activeElement?.blur?.()}catch(_){}btn.textContent="■";
 let remain=origin;
 const timer=setInterval(()=>{
   remain=Math.max(0,Math.ceil((Number(s._timerEndAt)-Date.now())/1000));s.time=remain;
   ...
   if(remain<=0){
     ...
     s.time=origin;s._timerOrigin=origin;s._timerEndAt=0;saveAll();signalTone();backgroundTimerNotice("Zeit beendet");renderLive();
...
function finishAndSaveWorkout(){
 if(!activeWorkout)return;
 // Normal run of an existing plan: no second question. Finish -> history -> summary.
